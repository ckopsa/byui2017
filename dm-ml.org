...
#+BEGIN_SRC python :results output :tangle neural_net.py
  ...
  for i in range(1000):
      for index, row in df_norm.iterrows():
          trained_classifier = train(row, outputize(targetVector[index], 3), trained_classifier, 3)[:-1][0]
  print trained_classifier[0]
  output = map(np.argmax, [classify(row, trained_classifier) for index, row in df_norm.iterrows()])
  ...
  #+RESULTS:
  [[0.88502308916241657, 1.8060592992466706, 0.63154927006937212,
  1.3703525247506094, 0.47994232331617676], [0.92162965568098121,
  1.3120830002514556, 1.4498923016439196, 1.2309626401726534,
  1.1320316965329682], [1.7043687857458418, 1.5244942425352885,
  0.5742307195257127, 0.9818885955651564, 1.6424403667152454],
  [2.0888920256399715, 1.1201062482151225, 1.2728713223456019,
  0.45970585477951181, 1.3621746553216556]]
  : 0.333333333333
