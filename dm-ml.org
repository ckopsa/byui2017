...
True
** Code
You will be adding to your experiment shell from the previous assignment, by
implementing a multi-layer perceptron neural network.

You will finish the assignment next week (and there is a second assessment for
that), however, to help encourage you to get started this week, you need to have
at least the following completed this week:

Create some type of data structure to hold a node (a.k.a. neuron).

Store the set of input weights for each node.

Provide a way to create a layer of nodes of any number (this should be easily
...
attributes) and have each node produce an output (i.e., 0 or 1) according to its
weights.
...
Notice that at this point, we are NOT implementing any sort of training or
learning in our algorithm. The point of this part of the assignment is to make
sure you have the basics of a network of nodes in place that can handle a real
instance from a dataset.

Next week, we will extend this classifier to handle multiple layers of nodes,
and implement the back-propagation algorithm for updating weights. Then, it will
...
#+BEGIN_SRC python :results output :tangle neural_net.py
  ...
  def calc_threshold(input_vector, input_weights):
      return int(sum([x * y for x, y in zip([-1] + input_vector, input_weights)]) > 0)
  input_vector = [2, 3, 4]
